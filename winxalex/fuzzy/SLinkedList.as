...
package winxalex.fuzzy
{
	...
	public class SLinkedList
	{
		 ...
		 * iterator is invalid.
		 * 
		 * @param node after which the
		 *            given data is inserted.
		 * @param obj The data to insert.
		 * 
		 * @return A singly linked list node wrapping the data.
		 */
		public function insertAfter(node:SListNode, obj:*):SListNode
		{
			
			if (node)
			{
				var newNode:SListNode = new SListNode(obj);
		...
		*/
		public function remove(node:SListNode):Boolean
		{
			if (! node) return false;
			
			var s:SListNode = head;
		...
		public function removeAfter(node:SListNode):void
		{
			var s:SListNode;
...
/*		public function sort(...sortOptions):void
		 ...
		 * @return node
		 */
		public function nodeOf(obj:*):SListNode
		{
		...
		public function splice(start:SListNode, deleteCount:uint = 0xffffffff, ...args):SLinkedList
		{
			//if (start) if (start.list != this) return null;
		...
		/**
		 * slice from node->next counting deleteCount
		 * @param	node
		 * @param	deleteCount
		 * @param	...args
		 */
		public function sliceAfter(node:SListNode, deleteCount:uint = 0xffffffff):void
		{
			var s:SListNode;
			var start:SListNode = node;
			var i:int;
			
			if (!start) return;
			
			if (start == tail)// || tail==head 
			{
			//removeTail();//no next item
			return;
			}
		...
		public function toString():String
		{
			...
			return "[SlinkedList, size = " + size + ":"+str+"]";// 
		}

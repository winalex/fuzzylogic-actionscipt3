package
{
	 ...
	 */
	public class Test1Comb extends Sprite
	{
		
		public function Test1Comb() 
		{
			...
			/**/
				...
				rule = new FuzzyRule( " IF Distance_to_Target IS Far  THEN Desirability IS Undesirable");
					fuz.addRule(rule);
						...
						rule = new FuzzyRule( "IF Distance_to_Target IS Medium THEN Desirability IS VeryDesirable ");
							fuz.addRule(rule);
								rule = new FuzzyRule( "IF Distance_to_Target IS Close THEN  Desirability IS Undesirable");
									fuz.addRule(rule);
									
							rule = new FuzzyRule( "IF Ammo_Status IS Low THEN  Desirability IS Desirable ");
								fuz.addRule(rule);
								rule = new FuzzyRule( "IF Ammo_Status IS Loads THEN Desirability IS  VeryDesirable");
									fuz.addRule(rule);
									
								
										rule = new FuzzyRule( "IF Ammo_Status IS Okey THEN  Desirability IS Desirable ");
										fuz.addRule(rule);
